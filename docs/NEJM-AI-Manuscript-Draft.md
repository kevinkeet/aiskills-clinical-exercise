...
- **AI arm:** an embedded, general-purpose chat assistant built on a frontier large language model (Anthropic Claude Opus 4.7), available in a side panel, with a one-click option to insert the case context into the conversation. The model was used **out of the box, with no system prompt or other configuration** — no instructions governing tone, length, tutoring, or learning — so that the intervention reflects a generic, off-the-shelf conversational assistant rather than a purpose-built educational tool. No separate login was required, and all messages were logged.
