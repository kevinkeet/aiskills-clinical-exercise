...
**Background.** Large language model (LLM) assistants can improve clinicians' performance on reasoning tasks in real time, but whether residents who work a clinical case *with* an AI assistant actually *learn* from it — that is, retain knowledge they can later deploy unaided — is unknown. Education and workforce commentary increasingly warns that AI may cause *de-skilling* (erosion of existing skills), *never-skilling* (trainees never acquiring them), or *mis-skilling* (internalizing AI errors). For resident physicians, whose clinical reasoning is real but still consolidating, the foremost concern is **de-skilling**; yet for cognitive tasks in medicine this concern rests largely on theory and analogy rather than randomized evidence. We tested it directly.
