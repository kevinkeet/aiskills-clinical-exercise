...
- **AI arm:** an embedded, general-purpose chat assistant built on a frontier large language model (Anthropic Claude Opus 4.7), available in a side panel, with a one-click option to insert the case context into the conversation. The assistant was used in essentially its default conversational form: a brief system prompt set a concise, high-yield clinical communication style but did not configure it to tutor, to question the resident Socratically, or otherwise to promote learning. No separate login was required, and all messages were logged. *(⚠ Drafting note: report the exact system prompt verbatim — ideally as a supplement. Note that it also asks the assistant to give targeted feedback when the resident shares their own reasoning, a mild pedagogical element to reconcile with the "default/general-purpose" description.)*
